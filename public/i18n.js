...
OWN.uk = {
  ...
  h1: 'Перевірка пошти',
  subtitle: 'SPF розкривається через усі include і рахується проти ліміту в десять запитів, вирівнювання і звітність DMARC, транспортний захист перевіряється на живому зʼєднанні',
  ph_host: 'example.com',
  ...
  note_transport: 'STARTTLS опортуністичний: відправник, у якого вирізали пропозицію шифрування, просто продовжить відкритим текстом. MTA-STS і DANE — це те, що перетворює можливість на гарантію.',
  ...
  fd_null_mx: 'Єдиний запис MX із пріоритетом 0, що вказує на корінь, — це RFC 7505 для «домен не надсилає і не приймає пошту». Свідоме рішення, і значно краще, ніж повна відсутність MX: без нього відправники відкочуються на адресний запис.',
  ...
  fd_spf_record_long: 'Довгі записи розбиваються на кілька рядків на дроті. Саме собою це нормально — одержувачі склеюють їх без роздільника, — але саме тут розбирачі, що зʼєднують їх пробілом, починають псувати запис.',
  ...
  fd_dkim_key_malformed: 'Значення p= не є коректним ключовим матеріалом. Будь-який підпис, зроблений ним, перевірку не пройде.',
  ...
  flag_dmarc_invalid_policy: 'Політика DMARC не є розпізнаваним значенням',
  ...
  fd_mtasts_missing: 'STARTTLS опортуністичний: відправник, який не побачив пропозиції шифрування, надішле відкритим текстом, а вирізати цю пропозицію легко будь-кому між серверами. MTA-STS публікує через HTTPS, що домен завжди говорить по TLS, і тим самим вимагає від атакувального зламати не рядок сесії, а вебовий PKI.',
  ...
  fd_mtasts_policy_unreachable: 'TXT-запис обіцяє політику, якої там немає, або сертифікат хоста політики не проходить перевірку. Оскільки весь механізм тримається на цьому сертифікаті HTTPS, збій тут означає, що відправники відкотяться на опортуністичний TLS.',
  ...
  flag_mtasts_policy_bad_mode: 'Режим політики не є розпізнаваним значенням',
  ...
  fd_mtasts_mx_not_in_policy: 'У наборі MX є хост, під який не підходить жоден шаблон mx: з політики. Будь-який відправник, що застосовує цю політику, відмовиться доставляти на цей хост — тобто пошта ламається саме в тих відправників, які поводяться акуратно.',
  ...
  fd_dane_missing: 'Записи TLSA закріплюють сертифікат, який зобовʼязаний надати поштовий сервер, спираючись на DNSSEC, а не на публічні засвідчувальні центри. З двох транспортних механізмів цей сильніший — і він вимагає підписаної зони, що зазвичай і є причина, з якої його не використовують.',
  ...
  fd_mx_certificate_not_trusted: 'Звичайний опортуністичний TLS сертифікати не перевіряє, тож сьогодні доставці це не заважає. Заважати почне рівно тоді, коли зʼявиться MTA-STS у режимі enforce або DANE.',
  ...
  fd_open_relay: 'Він прийняв листа від стороннього відправника сторонньому одержувачу. Будь-хто може слати через нього спам від вашого імені, і він опиниться в чорних списках протягом доби, якщо вже не там. Це лагодять сьогодні. (Перевірка зупинилася на RCPT TO і надіслала RSET — жодного листа надіслано не було.)',
