...
OWN.ru = {
  ...
  h1: 'Проверка почты',
  subtitle: 'SPF раскрыт через все include и сверен с лимитом в десять запросов, выравнивание и отчётность DMARC, транспорт проверен на живом соединении',
  ph_host: 'example.com',
  ...
  /* ---- пояснения ---- */
  note_spf: 'RFC 7208 разрешает десять термов, обращающихся к DNS, на всю проверку, считая внутри каждого include каждый вложенный include. За десятым получатель обязан вернуть permerror — а permerror означает, что SPF не применяется вовсе, ровно как если бы записи не было.',
  note_dmarc: 'При p=none не применяется ничего: письмо, не прошедшее выравнивание ни по SPF, ни по DKIM, доставляется как раньше. С этого правильно начинать — и неправильно на этом останавливаться.',
  note_transport: 'STARTTLS необязателен: отправитель, у которого вырезали объявление о шифровании, просто продолжит открытым текстом. Гарантией это делают MTA-STS и DANE.',
  ...
  cap_dmarc_reports_go_nowhere: 'внешние отчёты DMARC не разрешены',
  ...
  fd_duplicate_mx_host: 'Один хост встречается с разными приоритетами. Это не резервирование, а одна и та же машина, к которой попробуют подключиться дважды.',
  ...
  fd_mx_points_at_cname: 'RFC 2181 §10.3 требует, чтобы MX называл хост с адресными записями, а не CNAME. Часть отправителей справляется, часть отказывается, и кто из них как поступит, со временем меняется.',
  ...
  fd_spf_plus_all: '«+all» говорит, что отправлять от имени домена может любой хост где угодно. Это почти всегда непонимание того, что означает квалификатор, и это хуже отсутствия SPF: запись прямо ручается за того, кто подделывает отправителя.',
  ...
  flag_spf_unknown_mechanism: 'В SPF есть терм, которого никто не понимает',
  ...
  flag_dkim_key_malformed: 'Ключ DKIM не удалось разобрать',
  ...
  fd_dmarc_partial_percentage: 'pct= меньше 100 говорит получателям применять политику к этой доле неудачных писем, а с остальными обходиться мягче. Полезно при внедрении — и остаётся дырой всё время, пока стоит меньше ста.',
  ...
  flag_dmarc_external_reporting_unauthorised: 'Внешний адрес для отчётов не давал на это согласия',
  ...
  fd_mtasts_mode_none: 'mode=none отзывает политику. Так предусмотрен аккуратный вывод MTA-STS из обращения; если его оставить, запись просто перестаёт что-либо значить.',
  ...
  fd_mtasts_policy_no_mx: 'Политика без записей mx: не соответствует ни одному серверу, поэтому применяющим её отправителям доставлять некуда.',
  ...
  fd_port_25_blocked_from_here: 'Это про нашу сеть, не про вашу. Большинство хостеров закрывают исходящие соединения на порт 25 по умолчанию. Поэтому STARTTLS, сверку DANE с живым сертификатом и проверку релея сделать не удалось, и оценка не выставляется: считать её по тому, до чего дотянулись, было бы нечестно.',
  ...
  fd_no_starttls: 'Каждое доставленное на этот сервер письмо идёт через интернет незашифрованным и читается всем, что стоит на пути. Предложить STARTTLS стоит сертификата и одной строчки в настройках.',
  ...
  flag_starttls_legacy_protocol: 'Почтовый сервер соглашается на устаревшую версию TLS',
  ...
  fd_banner_reveals_version: 'Небольшой подарок тому, кто ищет хосты с известной ошибкой именно в этой версии. В приветствии можно написать что угодно.',

  flag_open_relay: 'Сервер пересылает почту для посторонних',
  fd_open_relay: 'Он принял письмо от постороннего отправителя постороннему получателю. Любой может слать через него спам от вашего имени, и он окажется в чёрных списках за считаные часы, если уже не там. Чинить это нужно сегодня же. (Проверка остановилась на RCPT TO и отправила RSET — никакого письма отправлено не было.)',
...
OWN.hi = {
  ...
  fd_mtasts_missing: 'STARTTLS वैकल्पिक है: जिस भेजने वाले को सर्वर की शिफ़रिंग की घोषणा दिखती ही नहीं वह खुले पाठ में भेज देता है, और दोनों सर्वरों के बीच खड़े किसी के लिए उस घोषणा को जवाब से हटा देना आसान है। MTA-STS HTTPS पर प्रकाशित करता है कि यह डोमेन हमेशा TLS बोलता है, जिससे उस हमले के लिए वेब PKI तोड़नी पड़ती है।',
...
OWN.ar = {
  ...
  fd_mtasts_missing: 'STARTTLS اختياري: المرسِل الذي لا يرى إعلان الخادم عن التشفير يرسل بنص مكشوف، وإزالة ذلك الإعلان سهلة على أي أحد بين الخادمين. وMTA-STS ينشر عبر HTTPS أن هذا النطاق يتكلّم TLS دائمًا، فيصير الهجوم مطالبًا بكسر البنية العامة لمفاتيح الويب بدلًا من ذلك.',
