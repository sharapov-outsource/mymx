...
**SPF раскрывается через все include, и результат сверяется с лимитом в десять.**

RFC 7208 §4.6.4 разрешает в записи десять терминов, обращающихся к DNS, —
...
При превышении получатель обязан вернуть `permerror`, а permerror означает, что
...
Цель с макросом вроде `exists:%{i}._spf.example.com` тратит своё обращение, но
...
бы здоровые записи за лимит на пустых обращениях.
...
Ещё: `p=none` называется своим именем — отчётность без всякого применения, —
...
**MTA-STS** забирается по HTTPS и сравнивается с настоящим набором MX. Почту для
узла, который есть в DNS, но отсутствует в политике, отвергнет каждый отправитель,
который эту политику соблюдает, — то есть отваливаются как раз аккуратные.
...
неподписанной зоне подаётся ровно как ложное спокойствие, которым она и является.
...
Однажды её уже совершили — за счёт банка.
...
| `SMTP_PORT_25` | — | `open` или `blocked`, чтобы пропустить исходящую пробу и сразу объявить результат |
